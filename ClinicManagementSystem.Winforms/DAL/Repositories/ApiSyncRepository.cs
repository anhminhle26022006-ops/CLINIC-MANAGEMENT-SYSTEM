    ...
    public class ApiSyncRepository
    {
        ...
        public async Task<ApiLocalUuidResultDTO> UpsertLocalEmployeeAsync(ApiEmployeeDTO employee)
        {
            using (var conn = DatabaseHelper.GetConnection())
            {
                await conn.OpenAsync();
                bool isNew = IsNewSchema(conn);
                await EnsureLocalEmployeeUuidSchemaAsync(conn, isNew);
                ...
                string uuidCol = isNew ? "EmployeeUUID" : "DoctorUUID";

                if (employee.SyncUuid.HasValue)
                {
                    int? existingId = await GetIdByUuidAsync(conn, table, idCol, uuidCol, employee.SyncUuid.Value);
                    if (existingId.HasValue)
                    {
                        await UpdateLocalEmployeeAsync(conn, existingId.Value, employee, uuid, isNew);
                        return new ApiLocalUuidResultDTO { Uuid = uuid, Inserted = false, Generated = false };
                    }
                }
...
END;";
        ...
        private static async Task EnsureLocalEmployeeUuidSchemaAsync(SqlConnection conn, bool isNewSchema)
        {
            if (isNewSchema)
            {
                string alterSql = @"
...
END;";
            ...
            {
                string alterSql = @"
IF COL_LENGTH('dbo.Doctors', 'DoctorUUID') IS NULL
BEGIN
    ALTER TABLE dbo.Doctors ADD DoctorUUID UNIQUEIDENTIFIER NULL;
END;";
                await ExecuteNonQueryAsync(conn, alterSql);

                string updateSql = "EXEC('UPDATE dbo.Doctors SET DoctorUUID = NEWID() WHERE DoctorUUID IS NULL');";
                await ExecuteNonQueryAsync(conn, updateSql);

                string indexSql = @"
IF NOT EXISTS (
    SELECT 1 FROM sys.indexes
    WHERE name = 'UQ_Doctors_UUID'
      AND object_id = OBJECT_ID('dbo.Doctors')
)
BEGIN
    EXEC('CREATE UNIQUE INDEX UQ_Doctors_UUID ON dbo.Doctors(DoctorUUID) WHERE DoctorUUID IS NOT NULL');
END;";
                await ExecuteNonQueryAsync(conn, indexSql);
            }
        }
        ...
        private static async Task<LocalIdentity> GetEmployeeByCodeAsync(SqlConnection conn, string employeeCode, bool isNewSchema)
        {
            if (string.IsNullOrWhiteSpace(employeeCode)) return null;

            string table = isNewSchema ? "Employees" : "Doctors";
            string idCol = isNewSchema ? "EmployeeID" : "DoctorID";
            string uuidCol = isNewSchema ? "EmployeeUUID" : "DoctorUUID";
            string codeCol = isNewSchema ? "EmployeeCode" : "DoctorCode";

            string sql = $"SELECT TOP 1 {idCol}, {uuidCol} FROM dbo.{table} WHERE {codeCol} = @Code";
            using (var cmd = new SqlCommand(sql, conn))
            {
        ...
        private static async Task<LocalIdentity> GetEmployeeByNameAsync(SqlConnection conn, string fullName, bool isNewSchema)
        {
            ...
            string idCol = isNewSchema ? "EmployeeID" : "DoctorID";
            string uuidCol = isNewSchema ? "EmployeeUUID" : "DoctorUUID";
            string nameCol = isNewSchema ? "FullName" : "Name";

            string sql = $"SELECT TOP 1 {idCol}, {uuidCol} FROM dbo.{table} WHERE {nameCol} = @Name";
            using (var cmd = new SqlCommand(sql, conn))
            {
