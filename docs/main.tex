...
\setminted{
    ...
    fontsize=\fontsize{7.6}{9}\selectfont,
    linenos,
    numbersep=4pt,
    breaklines=true,
    breakanywhere=true,
...
\newcommand{\demofigure}[3]{%
    \begin{figure}[H]
        \centering
        \safeincludegraphics[width=0.96\textwidth,height=0.72\textheight,keepaspectratio]{#1}
        \caption{#2}
        \smallskip
...
Phạm vi kỹ thuật dựa trên mã nguồn hiện có trong solution WinForms của đồ án. Các project chính gồm giao diện WinForms, BUS, DAL, DTO, Models, Core, ServicesExternal và script cơ sở dữ liệu CMS. Một số thư mục EF tồn tại ngoài solution chính, nhưng trong project WinForms đang khảo sát, thao tác dữ liệu chủ yếu được triển khai theo hướng ADO.NET với SqlClient.
...
\begin{itemize}
    ...
    \item Các main form theo vai trò giữ thanh menu, thông tin người dùng và vùng nội dung.
...
\hline
\endhead
DTO & Auth, Clinical, Facility, Scheduling & Đóng gói dữ liệu truyền giữa UI, BUS và DAL. \\
\hline
BUS & Services, Interfaces & Kiểm tra nghiệp vụ, điều phối repository và trả kết quả cho UI. \\
\hline
DAL & Repositories, DataContext & Tạo truy vấn SQL, truyền tham số, đọc dữ liệu và map sang DTO. \\
\hline
Core & Identity, Constants, Session, Utils & Lưu vai trò, trạng thái, session và tiện ích dùng chung. \\
\hline
\end{longtable}
...
Phân hệ tích hợp ngoài được tổ chức trong nhóm ApiSync ở tầng BUS và DAL. Repository đồng bộ có các phương thức đọc dữ liệu từ Supabase/SheetDB, upsert bệnh nhân, nhân viên, yêu cầu xét nghiệm, yêu cầu hình ảnh và kết quả vào SQL Server cục bộ. Form \texttt{MockApiSyncForm} dùng để mô phỏng thao tác đồng bộ và hiển thị lỗi khi dữ liệu API thiếu UUID hoặc JSON sai định dạng.
...
Ứng dụng Desktop đọc cấu hình từ \texttt{App.config}. Nhóm cấu hình chính gồm thông tin Supabase, SheetDB, backend trung gian và khóa PayOS. Việc gom cấu hình vào \texttt{App.config} giúp tránh hard-code endpoint trong form, đồng thời dễ thay đổi môi trường demo.
...
\begin{enumerate}
    \item Gọi các hàm đọc dữ liệu bệnh nhân và nhân viên từ Supabase/SheetDB.
    ...
    \item Bổ sung dữ liệu nhân viên: tự điền tên khoa, role bác sĩ theo mã và trạng thái đang làm việc nếu thiếu.
...
\begin{enumerate}
    ...
    \item Form ưu tiên gọi backend thông qua endpoint tạo thanh toán PayOS.
    ...
    \item WinForms kiểm tra trạng thái qua endpoint backend theo mã đơn hàng hoặc nhận cơ chế realtime nếu backend triển khai SignalR/WebSocket trong tương lai.
...
\demofigure{resources/csdl.png}
{Sơ đồ cơ sở dữ liệu của hệ thống}
{Hình minh họa cấu trúc dữ liệu tổng thể, trong đó các nhóm bảng bệnh nhân, lịch hẹn, lượt khám, cận lâm sàng, dược, thanh toán và phân quyền được liên kết bằng khóa chính, khóa ngoại. Sơ đồ này giúp đối chiếu nhanh giữa mô tả nghiệp vụ và script \texttt{Database/CMS.sql}.}
...
\hline
\makecell[l]{\texttt{EmployeeShifts}\\\texttt{ShiftID}} & \texttt{int} & Ca làm mà nhân viên đăng ký hoặc được phân công. \\
\hline
\makecell[l]{\texttt{DoctorSchedules}\\\texttt{RoomID}} & \texttt{int} & Phòng khám bác sĩ sử dụng trong lịch làm. \\
\hline
\end{longtable}
...
\hline
\makecell[l]{\texttt{Appointments}\\\texttt{AppointmentID}} & \texttt{int} & Khóa chính lịch hẹn. \\
\hline
\makecell[l]{\texttt{Appointments}\\\texttt{PatientID}} & \texttt{int} & Bệnh nhân được đặt lịch. \\
\hline
\makecell[l]{\texttt{Appointments}\\\texttt{DoctorID}} & \texttt{int} & Bác sĩ phụ trách lịch khám. \\
\hline
\makecell[l]{\texttt{Appointments}\\\texttt{RoomID}} & \texttt{int} & Phòng khám dự kiến. \\
\hline
\makecell[l]{\texttt{Appointments}\\\texttt{Status}} & \texttt{nvarchar} & Trạng thái lịch: chờ, đã check-in, hủy, hoàn tất. \\
\hline
\makecell[l]{\texttt{PatientQueues}\\\texttt{QueueID}} & \texttt{int} & Khóa chính hàng chờ. \\
\hline
\makecell[l]{\texttt{PatientQueues}\\\texttt{Status}} & \texttt{nvarchar} & Trạng thái trong hàng chờ: waiting, in-progress, done. \\
\hline
\end{longtable}
...
\hline
\endhead
\makecell[l]{\texttt{Encounters}\\\texttt{EncounterID}} & \texttt{int} & Khóa chính của lượt khám. \\
\hline
\makecell[l]{\texttt{Encounters}\\\texttt{PatientID}} & \texttt{int} & Bệnh nhân của lượt khám. \\
\hline
\makecell[l]{\texttt{Encounters}\\\texttt{DoctorID}} & \texttt{int} & Bác sĩ phụ trách. \\
\hline
\makecell[l]{\texttt{MedicalRecords}\\\texttt{RecordID}} & \texttt{int} & Hồ sơ bệnh án gắn với encounter. \\
\hline
\makecell[l]{\texttt{VitalSigns}\\\texttt{VitalID}} & \texttt{int} & Bản ghi sinh hiệu do điều dưỡng nhập. \\
\hline
...
\hline
Imaging request ID & \texttt{int} & Yêu cầu chẩn đoán hình ảnh. \\
\hline
...
\hline
Stock quantity & \texttt{int} & Số lượng tồn kho. \\
\hline
Prescription ID & \texttt{int} & Khóa chính đơn thuốc. \\
\hline
Prescription quantity & \texttt{int} & Số lượng thuốc bác sĩ kê. \\
\hline
Prescription dosage & \texttt{nvarchar} & Liều dùng hướng dẫn cho bệnh nhân. \\
\hline
...
\hline
\endhead
WinForms UI & Project giao diện, chứa entry point, main form, form con, user control và controller. \\
\hline
...
\hline
Receptionist & Patient, Appointment, Schedule, WaitingList, Payment & Màn hình nghiệp vụ của lễ tân, nạp động vào main form lễ tân. \\
\hline
Nurse & Tổng quan, sinh hiệu, hàng chờ, phòng, ca làm & Hỗ trợ nhập dữ liệu trước khám và theo dõi hàng chờ. \\
\hline
Technician & Requests, upload MRI/PDF, lab result, records, shifts & Màn hình xử lý request cận lâm sàng và đồng bộ dữ liệu kiểm thử. \\
\hline
Pharmacy & Overview, prescription, catalog, inventory & Màn hình cấp phát thuốc, danh mục thuốc và quản lý tồn kho. \\
\hline
ERM dùng chung & Overview, history, lab, imaging, prescription, PACS viewer & Thành phần hồ sơ bệnh án dùng chung, phục vụ bác sĩ và các màn hình cần xem hồ sơ. \\
\hline
WorkingShifts & Form và user control đăng ký/xem ca làm & Tái sử dụng chức năng ca làm cho nhiều vai trò. \\
\hline
...
Tầng DTO được chia theo domain để dữ liệu truyền giữa các lớp rõ ràng hơn. Một số nhóm tiêu biểu gồm:

\begin{itemize}
    \item Auth: DTO người dùng và phiên đăng nhập.
    \item Clinical: DTO bệnh nhân, hồ sơ khám, thanh toán và kết quả cận lâm sàng.
    \item Scheduling: DTO lịch hẹn, ca làm và phân công.
\end{itemize}

DTO giúp giao diện không phụ thuộc trực tiếp vào \texttt{DataTable} hoặc entity database.
...
Tầng DAL hiện tại được chia thành interface và repository theo domain. Ví dụ, nhóm xác thực xử lý đăng nhập, nhóm bệnh nhân xử lý hồ sơ hành chính, nhóm chỉ định xử lý dịch vụ cận lâm sàng và nhóm kỹ thuật viên xử lý request. Cách chia này làm rõ mỗi repository phụ trách một nhóm bảng và một nhóm truy vấn.
...
\hline
Clinical & Patient, Encounter, ERM, Payment, Prescription, VitalSigns & Truy vấn dữ liệu khám bệnh, hồ sơ, thanh toán, đơn thuốc. \\
\hline
Facility & Department, Room, Inventory, Supplier & Khoa, phòng, tồn kho, nhà cung cấp. \\
\hline
Scheduling & Appointment, DoctorSchedule, EmployeeShift, WorkShift & Lịch hẹn, lịch bác sĩ, ca làm. \\
\hline
Technician & Request, Shift & Danh sách request, upload kết quả, ca kỹ thuật viên. \\
\hline
Integrations & ApiSync & Gọi Supabase/SheetDB, patch UUID, upsert local, kéo request ngoài. \\
\hline
\end{longtable}
...
Khi thêm EF, project nên tách rõ nhóm DataContext, Entities và Repository EF. \texttt{DbContext} chỉ biết entity và connection string; repository EF chịu trách nhiệm query; BUS vẫn chỉ nhận DTO.
...
Lợi ích của controller là làm code-behind của user control ngắn hơn. Ví dụ \texttt{PaymentDetail} chỉ cần gọi controller để lấy chi tiết hóa đơn, bind dữ liệu hoặc cập nhật trạng thái thanh toán. Nếu sau này thay đổi logic tính tiền ở BUS/DAL, giao diện ít bị ảnh hưởng.
...
Các dashboard chính của lễ tân, bác sĩ, điều dưỡng, kỹ thuật viên và dược sĩ được xem như shell của từng vai trò. Shell gồm menu trái, vùng tiêu đề và panel nội dung. Khi người dùng bấm menu, shell không mở thêm cửa sổ rời rạc mà thay user control trong panel nội dung. Đây là kỹ thuật WinForms quan trọng vì nó tạo cảm giác ứng dụng có một workspace thống nhất, tương tự các phần mềm Desktop quản lý chuyên nghiệp.
...
Phân hệ quản trị viên tập trung vào dữ liệu nền và cấu hình vận hành của phòng khám. Các màn hình trong nhóm này dùng cho việc quan sát chỉ số tổng quan, quản lý nhân viên, chuyên khoa, ca trực, ca làm việc, thống kê và tài khoản người dùng. Đây là nhóm chức năng có tác động trực tiếp đến dữ liệu hệ thống nên giao diện cần ưu tiên bố cục rõ, bảng dữ liệu dễ lọc và các nút thao tác được đặt theo từng ngữ cảnh.

\demofigure{resources/admin/1.AD_TONGQUAN.jpg}
{Demo giao diện tổng quan quản trị viên}
{Màn hình tổng quan hiển thị các chỉ số chính của hệ thống, giúp quản trị viên nắm nhanh tình hình nhân sự, lịch hẹn, bệnh nhân và các hoạt động vận hành trong ngày.}

\demofigure{resources/admin/2.AD_QUANLYNHANVIEN.jpg}
{Demo giao diện quản lý nhân viên}
{Giao diện quản lý nhân viên dùng bảng dữ liệu làm trung tâm, hỗ trợ tra cứu, thêm, cập nhật và kiểm tra thông tin nhân sự theo vai trò, khoa phòng hoặc trạng thái làm việc.}

\demofigure{resources/admin/3.AD_QUANLYCHUYENKHOA.jpg}
{Demo giao diện quản lý chuyên khoa}
{Màn hình chuyên khoa cho phép quản trị viên theo dõi danh sách khoa, cập nhật thông tin khoa và chuẩn hóa dữ liệu nền phục vụ luồng đặt lịch khám.}

\demofigure{resources/admin/4.AD_QUANLYCATRUC.jpg}
{Demo giao diện quản lý ca trực}
{Màn hình ca trực hỗ trợ cấu hình khung giờ và trạng thái ca, làm cơ sở để các vai trò bác sĩ, điều dưỡng, kỹ thuật viên và dược sĩ đăng ký hoặc được phân công lịch làm việc.}

\demofigure{resources/admin/5.AD_CALAMVIEC.jpg}
{Demo giao diện ca làm việc của quản trị viên}
{Giao diện ca làm việc thể hiện danh sách ca theo thời gian, giúp quản trị viên kiểm tra lịch nhân sự, theo dõi đăng ký ca và xử lý các thay đổi phát sinh.}

\demofigure{resources/admin/6.AD_THONGKE.jpg}
{Demo giao diện thống kê quản trị}
{Màn hình thống kê tổng hợp dữ liệu vận hành thành biểu đồ hoặc chỉ số, phục vụ đánh giá tình hình khám bệnh, doanh thu, số lượt hẹn và hiệu suất sử dụng nguồn lực.}

\demofigure{resources/admin/7.AD_QUANLYTAIKHOAN.jpg}
{Demo giao diện quản lý tài khoản}
{Màn hình tài khoản tập trung vào thông tin đăng nhập, vai trò và trạng thái kích hoạt, giúp quản trị viên kiểm soát quyền truy cập của từng người dùng trong hệ thống.}
...
\hline
\endhead
Quản lý bệnh nhân & TextBox tìm kiếm, DataGridView, Button thêm/sửa & Tra cứu nhanh bệnh nhân và cập nhật hồ sơ hành chính. \\
\hline
Đặt lịch khám & FlowLayoutPanel khoa/bác sĩ, Button slot giờ, Dialog xác nhận & Tạo lịch hẹn theo khoa, bác sĩ và thời gian khám. \\
\hline
...
\demofigure{resources/reception/1.REPCEP_TONGQUAN.jpg}
{Demo giao diện tổng quan lễ tân}
{Màn hình tổng quan giúp lễ tân xem nhanh các tác vụ đầu ngày như số lịch hẹn, bệnh nhân chờ, thanh toán và các nhắc việc cần xử lý.}

\demofigure{resources/reception/2.RECEP_QUANLYBENHNHAN.jpg}
{Demo giao diện quản lý bệnh nhân của lễ tân}
{Giao diện quản lý bệnh nhân đặt ô tìm kiếm và bảng danh sách ở vị trí trung tâm, phù hợp với thao tác tra cứu, thêm mới hoặc cập nhật hồ sơ hành chính.}

\demofigure{resources/reception/3.RECEP_DATLICHKHAM.png}
{Demo giao diện đặt lịch khám}
{Màn hình đặt lịch thể hiện luồng chọn bệnh nhân, chuyên khoa, bác sĩ, ngày khám và khung giờ. Các bước lựa chọn được bố trí tuần tự để giảm nhầm lẫn khi tạo lịch hẹn.}

\demofigure{resources/reception/4.RECEP_THANHTOAN.png}
{Demo giao diện thanh toán tại quầy}
{Màn hình thanh toán hiển thị danh sách hóa đơn chờ xử lý, chi tiết phí và nút xác nhận phương thức thanh toán, giúp lễ tân hoàn tất giao dịch sau khi bệnh nhân khám xong.}

\demofigure{resources/reception/5.RECEP_LICHKHAMHOMNAY.png}
{Demo giao diện lịch khám hôm nay}
{Giao diện lịch trong ngày cho phép lễ tân theo dõi trạng thái từng lịch hẹn và thực hiện check-in trực tiếp trên dòng dữ liệu khi bệnh nhân đến phòng khám.}

\demofigure{resources/reception/6.RECEP_HANGCHOKHAM.png}
{Demo giao diện hàng chờ khám}
{Màn hình hàng chờ thể hiện danh sách bệnh nhân đã check-in, trạng thái chờ và thông tin liên quan để lễ tân phối hợp với điều dưỡng, bác sĩ trong quá trình điều phối khám.}

\demofigure{resources/reception/7.RECEP_NHACLICHKHAM.jpg}
{Demo giao diện nhắc lịch khám}
{Màn hình nhắc lịch hỗ trợ rà soát các lịch sắp diễn ra hoặc cần liên hệ lại, giúp giảm tình trạng bệnh nhân quên lịch và hỗ trợ chăm sóc trước khám.}

\demofigure{resources/reception/8.RECEP_CALAMVIEC.jpg}
{Demo giao diện ca làm việc của lễ tân}
{Giao diện ca làm việc cho lễ tân xem lịch trực, trạng thái ca và các thông tin phân công theo ngày hoặc tuần.}

\demofigure{resources/reception/9.RECEP_PayOS_MOMO.png}
{Demo giao diện thanh toán PayOS và MoMo QR}
{Màn hình thanh toán QR minh họa luồng chuyển khoản trong ứng dụng, trong đó lễ tân có thể tạo mã thanh toán và kiểm tra trạng thái giao dịch trước khi cập nhật hóa đơn.}
...
Các thành phần ERM được chia theo nhóm tổng quan, lịch sử khám, xét nghiệm, hình ảnh, toa thuốc, thanh toán, tái khám và trình xem PACS. Controller của ERM gọi tầng BUS để lấy hồ sơ bệnh nhân theo UUID.
...
\demofigure{resources/doctor/1.BACSI_TONGQUAN.jpg}
{Demo giao diện tổng quan bác sĩ}
{Màn hình tổng quan của bác sĩ tập trung vào lịch khám, bệnh nhân đang chờ và các thông tin cần xử lý trong phiên làm việc.}

\demofigure{resources/doctor/2.BACSI_LICHKHAM.jpg}
{Demo giao diện lịch khám của bác sĩ}
{Giao diện lịch khám giúp bác sĩ theo dõi danh sách bệnh nhân theo ngày, trạng thái tiếp nhận và thứ tự ưu tiên trước khi mở hồ sơ khám.}

\demofigure{resources/doctor/3.BACSI_KHAMBENH.jpg}
{Demo giao diện khám bệnh}
{Màn hình khám bệnh là điểm thao tác chính của bác sĩ, nơi nhập triệu chứng, chẩn đoán, chỉ định cận lâm sàng và quyết định hướng xử lý tiếp theo.}

\demofigure{resources/doctor/4.BACSI_BENHAN.jpg}
{Demo giao diện bệnh án điện tử}
{Giao diện bệnh án điện tử gom các nhóm dữ liệu y khoa của bệnh nhân, giúp bác sĩ xem lịch sử khám, sinh hiệu, kết quả cận lâm sàng và thông tin điều trị liên quan.}

\demofigure{resources/doctor/5.BACSI_TOATHUOC.jpg}
{Demo giao diện kê toa thuốc}
{Màn hình toa thuốc hỗ trợ chọn thuốc, số lượng, liều dùng và ghi chú, sau đó chuyển dữ liệu sang phân hệ dược sĩ để cấp phát.}

\demofigure{resources/doctor/6.BACSI_CALAMVIEC.jpg}
{Demo giao diện ca làm việc của bác sĩ}
{Giao diện ca làm việc cho bác sĩ xem lịch trực, lịch khám tương ứng và thông tin phân công trong tuần.}
...
\demofigure{resources/nurse/1.NUR_TONGQUAN.jpg}
{Demo giao diện tổng quan điều dưỡng}
{Màn hình tổng quan điều dưỡng hiển thị nhanh số bệnh nhân chờ, công việc cần xử lý và trạng thái phiên làm việc hiện tại.}

\demofigure{resources/nurse/2.NUR_HANGCHO.jpg}
{Demo giao diện hàng chờ của điều dưỡng}
{Giao diện hàng chờ giúp điều dưỡng theo dõi bệnh nhân cần đo sinh hiệu, điều phối phòng và chuyển trạng thái trước khi bác sĩ khám.}

\demofigure{resources/nurse/3.NUR_CHISOHIEUSINh.jpg}
{Demo giao diện nhập chỉ số sinh hiệu}
{Màn hình sinh hiệu tập trung vào các trường đo lường như nhiệt độ, mạch, huyết áp, nhịp thở, chiều cao và cân nặng, kèm kiểm tra dữ liệu trước khi lưu.}

\demofigure{resources/nurse/4.NUR_BENHAN.jpg}
{Demo giao diện xem bệnh án của điều dưỡng}
{Giao diện bệnh án cho điều dưỡng xem thông tin liên quan đến bệnh nhân và lượt khám, phục vụ chuẩn bị dữ liệu trước khi chuyển sang bác sĩ.}

\demofigure{resources/nurse/5.NUR_CALAMVIEC.jpg}
{Demo giao diện ca làm việc của điều dưỡng}
{Màn hình ca làm việc thể hiện lịch trực của điều dưỡng, giúp theo dõi phân công và phối hợp với các vai trò khác trong ngày.}

\subsubsection{Giao diện kỹ thuật viên}

\texttt{TechnicianMainform} điều hướng đến các màn hình request, upload MRI, upload PDF, nhập kết quả lab, xem hồ sơ bệnh nhân, ca làm việc và công cụ seeding dữ liệu. Các màn hình này kế thừa hoặc sử dụng \texttt{TechnicianDashboardViewBase} để phát sự kiện điều hướng.
...
\demofigure{resources/tech/1.KTV_TỔNG QUAN.jpg}
{Demo giao diện tổng quan kỹ thuật viên}
{Màn hình tổng quan kỹ thuật viên hiển thị các request cận lâm sàng cần xử lý, trạng thái công việc và lối vào nhanh đến các tác vụ upload hoặc nhập kết quả.}

\demofigure{resources/tech/2.KTV_XETNGHIEM.jpg}
{Demo giao diện danh sách xét nghiệm}
{Giao diện xét nghiệm cho phép kỹ thuật viên theo dõi request, chọn bản ghi cần xử lý và chuyển sang nhập kết quả lab tương ứng.}

\demofigure{resources/tech/3.KTV_MRX.jpg}
{Demo giao diện xử lý MRI/X-Ray}
{Màn hình MRI/X-Ray phục vụ các request hình ảnh, giúp kỹ thuật viên chọn hồ sơ, kiểm tra thông tin bệnh nhân và chuẩn bị file kết quả.}

\demofigure{resources/tech/4.KTV_TAILENPDF.jpg}
{Demo giao diện tải lên file PDF}
{Giao diện tải PDF minh họa bước chọn file cận lâm sàng bằng hộp thoại hệ thống, hiển thị đường dẫn và kích hoạt thao tác upload khi file hợp lệ.}

\demofigure{resources/tech/5.KTV_KQLAB.jpg}
{Demo giao diện nhập kết quả lab}
{Màn hình kết quả lab cho phép nhập thông số xét nghiệm, ghi chú và cập nhật trạng thái request để bác sĩ có thể đọc lại trong bệnh án điện tử.}

\demofigure{resources/tech/6.KTV_BENHAN.jpg}
{Demo giao diện xem bệnh án của kỹ thuật viên}
{Giao diện bệnh án giúp kỹ thuật viên kiểm tra bối cảnh bệnh nhân và lượt khám liên quan trước khi cập nhật kết quả cận lâm sàng.}

\demofigure{resources/tech/7.KTV_CALAMVIEC.jpg}
{Demo giao diện ca làm việc của kỹ thuật viên}
{Màn hình ca làm việc hiển thị lịch trực của kỹ thuật viên, hỗ trợ theo dõi phân công và khối lượng request trong phiên làm việc.}

\subsubsection{Giao diện dược sĩ}

\texttt{PharmacyMainform} điều hướng đến các màn hình tổng quan, cấp phát toa thuốc, danh mục thuốc, quản lý kho và ca làm việc. Các view kế thừa \texttt{PharmacyDashboardViewBase}, giúp thống nhất sự kiện điều hướng.
...
\demofigure{resources/pharmacist/1.DUOC_SI_TỔNG QUAN DƯỢC.jpg}
{Demo giao diện tổng quan dược sĩ}
{Màn hình tổng quan dược sĩ hiển thị các đơn thuốc chờ cấp phát, cảnh báo tồn kho và lối vào nhanh đến danh mục thuốc hoặc quản lý kho.}

\demofigure{resources/pharmacist/2.DUOC_SI_TOA THUỐC.jpg}
{Demo giao diện toa thuốc chờ cấp phát}
{Giao diện toa thuốc giúp dược sĩ xem danh sách đơn, mở chi tiết thuốc, kiểm tra số lượng và xác nhận cấp phát cho bệnh nhân.}

\demofigure{resources/pharmacist/3.DUOC_SI_DANHMUCTHUOC.png}
{Demo giao diện danh mục thuốc}
{Màn hình danh mục thuốc trình bày thông tin thuốc theo danh sách hoặc card, hỗ trợ tìm kiếm, xem hàm lượng, đơn vị tính và thao tác chỉnh sửa.}

\demofigure{resources/pharmacist/4.DUOC_SI_QUANLYKHO.png}
{Demo giao diện quản lý kho thuốc}
{Giao diện kho thuốc tập trung vào số lượng tồn, trạng thái cảnh báo và các thao tác cập nhật nhập xuất, giúp hạn chế cấp phát khi không đủ thuốc.}

\demofigure{resources/pharmacist/5.DUOC_SI_CALAMVIEC.jpg.png}
{Demo giao diện ca làm việc của dược sĩ}
{Màn hình ca làm việc cho dược sĩ theo dõi lịch trực và phối hợp với quầy tiếp nhận, bác sĩ trong quá trình cấp phát thuốc.}

\subsubsection{Cài đặt lịch làm việc dùng chung}

Ngoài các phân hệ riêng, hệ thống có nhóm màn hình ca làm dùng chung. Nhóm này gồm các view theo tuần, ngày, tháng, card ca làm, form chi tiết ca và form đăng ký ca, dùng để mô phỏng lịch làm việc của nhân viên. Đây là phần nên trình bày trong chương cài đặt vì nó thể hiện kỹ thuật tái sử dụng user control giữa nhiều vai trò.
...
Ở mức cài đặt, \texttt{ApiSyncRepository} đọc endpoint từ \texttt{App.config}. Với Supabase, repository gọi REST API của các bảng bệnh nhân và nhân viên, đồng thời thêm header xác thực. Với SheetDB, repository gọi URL đã được ghép tên sheet bằng hàm \texttt{WithSheetName}. Cả hai nguồn đều deserialize JSON về DTO bằng \texttt{System.Text.Json}; phần cấu hình đọc không phân biệt hoa thường và cho phép đọc số từ chuỗi để xử lý dữ liệu API linh hoạt hơn.

\demofigure{resources/API/csv_sheet_api.jpg}
{Demo dữ liệu CSV/Sheet dùng cho đồng bộ API}
{Hình minh họa nguồn dữ liệu dạng bảng được dùng làm đầu vào cho SheetDB. Cấu trúc này gần với Google Sheet nên phù hợp để nhóm mô phỏng dữ liệu bệnh nhân hoặc nhân viên từ nguồn ngoài.}

\demofigure{resources/API/sheetdb.jpg}
{Demo cấu hình SheetDB}
{Màn hình SheetDB cho thấy cách dữ liệu sheet được công bố thành API REST, giúp ứng dụng WinForms có thể đọc dữ liệu JSON thay vì thao tác trực tiếp với file bảng tính.}

\demofigure{resources/API/supabase.jpg}
{Demo giao diện bảng dữ liệu Supabase}
{Hình minh họa nguồn dữ liệu Supabase dùng trong luồng đồng bộ, nơi dữ liệu được lưu theo bảng và truy cập qua REST endpoint.}

\demofigure{resources/API/supabase_sql.jpg}
{Demo truy vấn SQL trên Supabase}
{Màn hình SQL của Supabase hỗ trợ kiểm tra dữ liệu, đối chiếu bản ghi và xác minh kết quả đồng bộ trước khi dữ liệu được đưa về SQL Server local.}
...
Phân hệ kỹ thuật viên có thêm thao tác đồng bộ request từ Supabase. Khác với đồng bộ bệnh nhân/nhân viên, request cận lâm sàng cần ghép nhiều bảng: encounter để biết bệnh nhân, employee để biết bác sĩ, services để biết loại dịch vụ, results để lấy kết quả hoặc URL file. Luồng đồng bộ request tạo các dictionary tra cứu để xử lý nhanh, sau đó gọi hàm upsert request local.
...
Ở mức cài đặt, \texttt{PaymentDetail} cho phép người dùng chọn tiền mặt hoặc chuyển khoản. Nếu chọn chuyển khoản, form mở \texttt{PayOsPaymentForm}. Form này tạo payment link, render QR và kiểm tra trạng thái. Sau khi người dùng xác nhận, controller thanh toán gọi BUS để cập nhật trạng thái trong SQL Server.

\demofigure{resources/PayOs/payos.jpg}
{Demo giao diện tạo thanh toán PayOS}
{Màn hình PayOS minh họa bước tạo yêu cầu thanh toán, hiển thị mã QR hoặc đường dẫn thanh toán để bệnh nhân thực hiện chuyển khoản.}

\demofigure{resources/PayOs/payos_sta.jpg}
{Demo giao diện kiểm tra trạng thái PayOS}
{Màn hình trạng thái PayOS giúp theo dõi giao dịch đang chờ, đã thanh toán hoặc bị hủy, từ đó quyết định thời điểm cập nhật hóa đơn trong SQL Server.}
...
\hline
Polling trạng thái & Timer WinForms & Desktop gọi API kiểm tra trạng thái 5 giây/lần để cập nhật UI. \\
\hline
...
\hline
\endhead
Đăng nhập & LoginForm, MainformRole & UserBUS, UserDAL, session & Session được tạo, dashboard đúng vai trò được mở. \\
\hline
Quản lý bệnh nhân & PatientManagement, form thêm mới & PatientBUS, PatientDAL & Bệnh nhân được thêm/sửa/tìm kiếm trong SQL Server. \\
\hline
Tạo lịch khám & Đặt lịch, xác nhận lịch & AppointmentBUS, DoctorScheduleBUS & Lịch hẹn có bác sĩ, phòng, ngày giờ và trạng thái ban đầu. \\
\hline
Check-in & ScheduleToday & AppointmentBUS, QueueBUS & Lịch hẹn chuyển trạng thái, hàng chờ được tạo. \\
\hline
Khám bệnh & DoctorMainform, ERM & ERMBus, EncounterDAL, PrescriptionDAL & Encounter, chẩn đoán, chỉ định, toa thuốc được lưu. \\
\hline
Nhập sinh hiệu & Màn hình sinh hiệu & VitalSignBUS, VitalSignDAL & Sinh hiệu được gắn với lượt khám/bệnh nhân. \\
\hline
Xử lý request & Request, lab, upload MRI & TechnicianRequestBUS, DAL kỹ thuật viên & Kết quả lab/file hình ảnh được cập nhật, request đổi trạng thái. \\
\hline
Cấp phát thuốc & Cấp phát, card thuốc & PharmacyBUS, MedicineDAL, InventoryDAL & Đơn thuốc được cấp phát, tồn kho cập nhật. \\
\hline
Thanh toán & Payment, PayOS form & PaymentController, PaymentBUS, PaymentDAL & Hóa đơn và lịch sử thanh toán được cập nhật. \\
\hline
Đồng bộ API & MockApiSyncForm, seeder & ApiSyncBUS, ApiSyncRepository & Dữ liệu Supabase/SheetDB được hợp nhất vào SQL Server local. \\
\hline
\end{longtable}
...
\hline
Đã xác nhận & Form xác nhận & \texttt{AppointmentDTO} & Gọi BUS lưu lịch hẹn xuống SQL Server. \\
\hline
\end{longtable}
...
\hline
\texttt{Processing} & Kỹ thuật viên & Màn hình request & Request đã được nhận xử lý hoặc đang nhập kết quả. \\
\hline
...
\begin{itemize}
    \item Một số namespace và cách đặt tên chưa hoàn toàn thống nhất, ví dụ có lớp dùng \path|BUS.Services|, có lớp dùng \texttt{BUS}.
...
\clearpage
\section*{PHỤ LỤC B. BẢNG, BIỂU}
\phantomsection
\addcontentsline{toc}{section}{Phụ lục B. Bảng, biểu}
