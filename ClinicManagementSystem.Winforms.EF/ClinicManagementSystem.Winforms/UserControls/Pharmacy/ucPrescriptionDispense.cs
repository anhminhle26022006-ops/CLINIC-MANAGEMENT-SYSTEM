...
namespace ClinicManagementSystem.Winforms.UserControls.Pharmacy
{
    public partial class ucPrescriptionDispense : PharmacyDashboardViewBase
    {
        ...
        private readonly PrescriptionBUS _prescriptionBUS;
        ...
        public ucPrescriptionDispense(CMSDbContext context, UserDTO currentUser) : this()
        {
            _prescriptionBUS = new PrescriptionBUS(context);
        }
        ...
        private void LoadPrescriptions()
        {
            try
            {
                pendingPrescriptions = _prescriptionBUS.GetPendingPrescriptions();
            }
            catch (Exception ex)
            {
        ...
        {
            string term = txtPrescriptionSearch.Text == SearchPlaceholder ? string.Empty : txtPrescriptionSearch.Text.Trim().ToLower();
        ...
        {
            if (status == "Tất cả trạng thái") return true;
            if (status == "Chờ cấp phát") return IsWaitingStatus(prescription);
            if (status == "Đang chuẩn bị") return IsPreparingStatus(prescription);
        ...
        {
            if (sender is not ucPharmacyPrescriptionQueueCard card) return;
            var prescription = visiblePrescriptions.FirstOrDefault(p => p.PrescriptionID == card.PrescriptionId);
            if (prescription != null) SelectPrescription(prescription);
        }
        ...
        private void SelectPrescription(PrescriptionDTO prescription)
        {
            ...
            lblAvatar.Text = string.IsNullOrWhiteSpace(prescription.PatientName) ? "P" : prescription.PatientName.Substring(0, 1).ToUpper();

            lblDiagnosis.Text = string.IsNullOrWhiteSpace(prescription.Diagnosis) ? "Chưa cập nhật chẩn đoán." : prescription.Diagnosis;
            lblNoteContent.Text = string.IsNullOrWhiteSpace(prescription.DoctorNotes) ? "Không có lời dặn đặc biệt." : prescription.DoctorNotes;

            bool hasAllergy = HasAllergy(prescription.PatientAllergies);
            lblAllergy.Text = hasAllergy ? "Dị ứng: " + prescription.PatientAllergies : "Không phát hiện dị ứng thuốc.";
        ...
        private static string BuildPatientMeta(PrescriptionDTO prescription)
        {
            ...
            string gender = string.IsNullOrWhiteSpace(prescription.PatientGender) ? "Chưa cập nhật giới tính" : prescription.PatientGender;
            string patientCode = string.IsNullOrWhiteSpace(prescription.PatientCode) ? "Chưa có mã BN" : prescription.PatientCode;
        ...
        private void BtnDispense_Click(object sender, EventArgs e)
        {
            ...
            var result = MessageBox.Show("Xác nhận cấp phát thuốc cho bệnh nhân " + selectedPrescription.PatientName + "?",
                "Cấp phát thuốc", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (result != DialogResult.Yes) return;

            try
            {
                _prescriptionBUS.ValidateStock(selectedPrescription.Items);
                foreach (var item in selectedPrescription.Items)
                    _prescriptionBUS.DispenseMedicine(item.MedicineID, item.Quantity);
                _prescriptionBUS.CompletePrescription(selectedPrescription.PrescriptionID);
                dispensedTodayCount++;

                MessageBox.Show("Đã cấp phát thành công đơn thuốc #" + selectedPrescription.PrescriptionID + ".",
                    "Cấp phát thành công", MessageBoxButtons.OK, MessageBoxIcon.Information);
                LoadPrescriptions();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Không thể cấp phát đơn thuốc:\n" + ex.Message, "Lỗi cấp phát", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private static bool HasAllergy(string allergy) => !string.IsNullOrWhiteSpace(allergy) && allergy.Trim() != "Không" && allergy.Trim() != "None";
        private static bool IsWaitingStatus(PrescriptionDTO prescription) => prescription.Status == "Chờ cấp phát" || prescription.Status == "Pending";
        private static bool IsPreparingStatus(PrescriptionDTO prescription) => !IsWaitingStatus(prescription) && !IsCompletedStatus(prescription.Status);
        private static bool IsCompletedStatus(string status) => status == "Completed" || status == "Đã cấp phát" || status == "Dispensed";
    }
}