    ...
    partial class ucShiftRequestManagement
    {
        ...
        private void InitializeComponent()
        {
            ...
            kpiFlow.Resize += (s, e) =>
            {
                ...
                int gap = 16, w = (kpiFlow.Width - gap * 3) / 4;
                if (w <= 0) return;
                foreach (var c in cards) { c.Width = w; c.Height = 120; c.Margin = new Padding(0, 0, gap, 0); }
            };
            ...
            panelWarning.Paint += (s, e) =>
            {
                using var pen = new System.Drawing.Pen(Color.FromArgb(234, 179, 8), 1);
                e.Graphics.DrawRectangle(pen, 0, 0, panelWarning.Width - 1, panelWarning.Height - 1);
            };
            panelWarning.Resize += (s, e) =>
            {
            ...
            panelTabs.Paint += (s, e) =>
            {
                ...
                foreach (Control c in panelTabs.Controls)
                    if (c is Button b && b.Tag?.ToString() == "active")
                    {
                        using var blue = new System.Drawing.Pen(Color.FromArgb(37, 99, 235), 2);
                        e.Graphics.DrawLine(blue, b.Left, panelTabs.Height - 2, b.Right, panelTabs.Height - 2);
                    }
            };
        ...
        private Panel MakeKpiCard(string title, string value, string sub, Color accent, Color bg, bool hasBorder, string icon)
        {
            ...
            iconBox.Paint += (s, e) => e.Graphics.DrawString(icon, new Font("Segoe UI", 14F), new SolidBrush(accent), 8, 8);
